...
<template>
  <div class="relative w-full flex flex-col items-center">
    <div class="w-full h-[10rem] bg-neutral-600">

    </div>
    <div class="w-full h-[5rem] flex items-center justify-between">
      <div>
  ...
  <div class="w-full h-[10rem] flex flex-col items-end justify-end gap-4">
    <p class="text-neutral-100 text-4xl font-bold">${{ product.price }}</p>
    <div class="w-full flex items-center justify-end gap-4">
      <Button @click="handleDelete" label="Delete"class="w-20 h-[2.5rem]" severity="danger"/>
      <Button @click="emit('edit')" label="Update" class="w-20 h-[2.5rem]" severity="info"/>
      <Button @click="emit('show-history', product)" label="History" class="w-40 h-[2.5rem]"/>
    </div>
  </div>
</template>