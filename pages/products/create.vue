...
<template>
  <div class="fixed bg-neutral-700 w-screen h-screen flex flex-col items-center sm:py-8 gap-8 overflow-y-auto">
    <Menu />
    <div v-if="loading" class="absolute top-1/2">
      <i class="pi pi-spin pi-spinner text-white"></i>
    </div>
    <div class="w-screen sm:w-[28rem] sm:h-[36rem] h-screen p-4 flex flex-col items-center gap-8 bg-neutral-800 sm:rounded-2xl"> 
      <p class="text-neutral-100 text-2xl font-semibold">Add new product</p>
      
      <div class="w-[80%] min-w-30 flex flex-col gap-8">
        ...
        <div class="w-full flex items-center justify-evenly">
          <p class="text-neutral-500 text-md font-semibold hidden sm:hidden-0">Set visibility:</p>
      ...
      <div class="flex w-96 justify-center gap-8 sm:mt-auto">
        <Button @click="navigateTo('/products')" label="Cancel" class="w-32" raised />
        <Button @click="handleCreateProduct" label="Submit" class="w-32" raised />
      </div>
    </div>
  </div>
</template>